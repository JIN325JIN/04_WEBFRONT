...
function check1(){
  ...
  //JS배열의 특징 : index별로 자료형을 다르게 지정할 수 있다.
  //하나의 배열로 여러개 데이터를 다룰 수 있다.
...
function check2(){
  ...
  //for문에서 증감문으로 i값이 변하니까 let으로 하기
  for(let i =0 ; i<= 3;i++ ){
    arr[i]=i*10
  }
  console.log("arr: ", arr)
...
//점심 메뉴로 초기화 된 배열 생성
const menu = ["제육볶음","돈까스","파스타","순대국","닭갈비","냉면","김밥",
  "치킨","떡볶이","햄버거","초밥"];//길이 11

  //menu 배열 index범위 내에서 난수 생성
  const randomNumber = Math.floor(Math.random()*menu.length);
  //math.floor() 이용시 소수점 이하가 버려져 정수 범위만 출력
  //0이상 10이하의 난수값 생성.
  //0부터 10까지 세면 11개임

  //난수번째 index요소 값을 화면에 출력
