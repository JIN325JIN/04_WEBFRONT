...
document.querySelector("#btn1").addEventListener("click",()=>{
  ...
  console.log(regExp2.exec(str3));//첫번째 java반환
...
document.querySelector("#btn2").addEventListener("click",()=>{
  ...
  //^(캐럿)
  const regExp2 =/^group/;//문자열 시작이 group인지 확인
...
document.getElementById("inputName").addEventListener("keyup",(e)=>{
  ...
  if(regExp.test(e.target.value)){//유효한 경우
    ...
    span.innerText="이름형식이 유효하지 않습니다.";
...
document.getElementById("inputPno").addEventListener("keyup",(e)=>{
  ...
  if(e.target.value.length==0){
    ...
    span.classList.remove("confirm","error");//일반 span태그
    return;
  }
  ...
  if(regExp.test(e.target.value)){
    ...
    span.innerText ="유효하지않은 주민등록번호 형식입니다.";
